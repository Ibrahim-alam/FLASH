...
def HPO_combined_client(Clfrs, i, N_i, HPO_ITER, target_dir, file):
    ...
    def hyperparameter_tuning(params):
        ...
        avg_loss=-np.mean(client_scores)
            
        return {"loss": avg_loss, "status": STATUS_OK, 'client_scores': client_scores, 'eval_time': time.time(), 'HP_dict':params}
    ...
    for k in range(HPO_ITER[i]):
        HP_dict.append(trials.results[k].get('HP_dict'))
    ...
    for j in range(N_Clients):
        temp=[]
        for k in range(HPO_ITER[i]):
            Clnt_acc_temp=trials.results[k].get('client_scores')
            temp.append(Clnt_acc_temp[j])
        HPs['Client_'+str(j+1)]=temp
...
def main_func_FLASH():
    ...
    while(p>0):
        ...
        for i in List_i:
            #%% HPO for each client
            HPs, HP_dict=HPO_combined_client(Clfrs,i,N_i,HPO_ITER,target_dir, file) # Calling the function to calculate HPs of HPO
            Full_performance[i].append(HPs)# Full_performance_Alg)
            Top_HPs_all_clsfr[i].append(HP_dict[HPs["indx"].iloc[0]])
            #%% Storing the best score (accuracy)
            temp2=-HPs['Accuracy'].iloc[0]
    ...
    for i in range(len(Clfrs)):
        ...
        Top_HP_index=len(Top_HPs_all_clsfr[i])-1
...
pkl.dump(All_Results_combined,open(file['DataSet_name'].iloc[0]+"_N"+str(N_Clients)+"_FLASH_C_All_HPO_Data_"+str(Data_Seed)+".p", "wb" ))
Full_Clsfr_Acc_DF.to_csv(file['DataSet_name'].iloc[0]+"_N"+str(N_Clients)+'_FLASH_C_Accuracy_'+str(Data_Seed)+'.csv')
